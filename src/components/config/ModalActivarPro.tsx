// Modal para ingresar un código de activación del Plan Pro.

import { useState } from 'react'
import { X, Lock, MessageCircle } from 'lucide-react'
import toast from 'react-hot-toast'
import { activarPlanPro } from '../../hooks/useConfig'
...
export function ModalActivarPro({ onClose }: Props) {
  ...
  const handleActivar = async (e: React.FormEvent) => {
    ...
    setCargando(true)
    const ok = await activarPlanPro(codigo)
    setCargando(false)
    if (ok) {
      toast.success('¡Plan Pro activado! Ya tienes acceso a domicilios')
      onClose()
    } else {
  ...
  const urlWhatsApp = `https://wa.me/${NUMERO_DESARROLLADOR}?text=${encodeURIComponent(
    `Hola! Quiero activar el Plan Pro del POS Tienda.\nNombre tienda: ${nombreTienda}`
  )}`
      ...
      <div className="bg-white w-full sm:max-w-sm rounded-t-2xl sm:rounded-2xl shadow-2xl">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-borde">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-acento/10 flex items-center justify-center">
              <Lock size={16} className="text-acento" />
            </div>
            <h2 className="font-display font-bold text-base text-texto">Activar Plan Pro</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-5 py-5 flex flex-col gap-4">
          <p className="text-sm text-suave leading-relaxed">
            Ingresa tu código de activación para desbloquear domicilios, catálogo público y vista del repartidor.
          </p>
          ...
          <div className="border-t border-borde pt-4">
            <p className="text-xs text-suave text-center mb-2">¿No tienes código?</p>
            <a
              href={urlWhatsApp}
              target="_blank"
